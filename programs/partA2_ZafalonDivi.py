...
def load_database(arguments: List[str]):
    ...
    parser.add_argument('-f', '--data_dir', dest='data_dir',
                        help='path of the directory containing data (data, citations and reviewers) csvs '
    ...
    with open(args.script_path, mode='r', encoding='utf8') as script:
        ...
        params = {'data_csv': args.data_csv or f'{args.data_dir}/data.csv',
                  'citations_csv': args.citations_csv or f'{args.data_dir}/citations.csv',
                  'reviewers_csv': args.reviewers_csv or f'{args.data_dir}/reviewers.csv'}
